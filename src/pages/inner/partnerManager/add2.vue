    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="demo-ruleForm">
      ...
      <div v-if="radio=='2'">
        <el-form-item label="姓名" prop="companyName">
          <el-input v-model="ruleForm.companyName" placeholder='请输入姓名' id="xx"></el-input>
        </el-form-item>
      ...
      <h1 class="form_table_h1">加盟与结算信息</h1>
        ...
        <!-- 选择加盟方式 -->
      <el-form :model="newFormObject" :rules="rules1" ref="newFormObject" label-width="150px" class="demo-ruleForm1">
        <div class="mutiFormSelect" v-bind:key="list.id" v-for="(list,index) of ruleForm.multiForm" id="isEmpty">
          ...
          <el-form-item label="加盟模式" prop="joinMode">
            <el-radio-group v-model="newFormObject.joinMode" @change="checkJoinMode" id="only">
          ...
          <!-- 1、当页面第一次加载打开时，从后台拿到数据list.joinMode的值为1，此时独家的内容显示出来
            ...
            -->
          <div v-if="newFormObject.joinMode=='1' || (radio=='2'||newFormObject.joinMode=='2'?false:true)">
            <el-form-item label="加盟地区" :id="'cityId'+ index" prop="cityId">
              <el-select v-model="newFormObject.cityId" placeholder="请选择">
                <el-option
            ...
            <el-form-item label="加盟日期" :id="'joinTime'+ index" prop="joinTime">
              <el-date-picker
                v-model="newFormObject.joinTime"
                placeholder="选择日期">
              </el-date-picker>            
            </el-form-item>
            <el-form-item label="认购车辆" :id="'subscriptionNum'+ index" prop="subscriptionNum">
              <el-input v-model="newFormObject.subscriptionNum" placeholder='请输入车辆数(单位：/辆)'></el-input><span style="margin-left:5px;">辆</span>
            </el-form-item>
            <el-form-item label="加盟资金" :id="'subscriptionMoney'+ index" prop="subscriptionMoney">
              <el-input v-model="newFormObject.subscriptionMoney" placeholder='请输入加盟资金(元)'></el-input><span style="margin-left:5px;">元</span>
            </el-form-item>
            ...
            <el-form-item label="授权费率" :id="'licenseFeeRate'+ index" prop="licenseFeeRate">
              <el-input max="100" min="0" v-model="newFormObject.licenseFeeRate" placeholder='请输入授权费率'></el-input><span style="margin-left:5px;">%</span>
            </el-form-item>
            <el-form-item label="结算周期" :id="'wType'+ index" prop="wType">
              <el-radio-group v-model="newFormObject.wType" @change="checkSettleType">
                ...
                <el-input :id="'circleDays'+ index" class="customInput" style="display:inline;width:200px;" v-show="list.wType=='自定义'"
                  v-model="newFormObject.circleDays"
                  placeholder="请输入正整数（天）">
                </el-input>  
              </el-radio-group>
            </el-form-item>
            <h1 class="form_table_h2">次周期结算上一个结算周期的收益，如果第一个周期不满一个结算周期也进行结算</h1>
            <el-form-item label="首次结算开始日期" :id="'firstDealDate'+ index" prop="firstDealDate">
                  <el-date-picker
                    :readonly="isHaveSettleOrders"
                    v-model="newFormObject.firstDealDate"
                    placeholder="选择日期">
                  </el-date-picker>           
            </el-form-item>
          ...
          <!-- 个人或者非独家时 -->
          <div v-if="newFormObject.joinMode=='2'|| radio=='2'">
            <el-form-item label="加盟地区" :id="'cityId'+ index" prop="cityId">
                <el-select v-model="newFormObject.cityId" placeholder="请选择">
                  <el-option
                    v-for="item in ruleForm.options"
            ...
            <el-form-item label="加盟资金" :id="'subscriptionMoney'+ index" prop="subscriptionMoney">
              <el-input v-model.number="newFormObject.subscriptionMoney" placeholder='请输入加盟资金(元)'></el-input><span style="margin-left:5px;">元</span>
            </el-form-item>
            <el-form-item style="position: relative; top: -22px; margin-bottom: 7px;">
              <div class="el-form-item__error" v-show="areaError">该地区已经存在加盟商，请重新选择</div>
            </el-form-item>
            <el-form-item label="运营管理费" :id="'manageFee'+ index" prop="manageFee">
                <el-input v-model.number="newFormObject.manageFee" placeholder='请输入运营管理费'></el-input><span style="margin-left:5px;">元</span>
            </el-form-item>
            <el-form-item label="结算日"
              :id="'settleDays'+index" prop="dayList">
              <el-checkbox-group v-model="newFormObject.dayList"  @change="checkSettleDays">
            ...
            <h1 class="form_table_h2">生成结算单后，此日期不允许修改</h1>
            <el-form-item label="后期分成比例" :id="'divisionPercent'+ index" prop="divisionPercent">
              <el-input v-model.number="newFormObject.divisionPercent" placeholder='请输入后期分成比例'></el-input><span style="margin-left:5px;">%</span>
            </el-form-item>
          </div>
        </div>
      </el-form>
        ...
        <!-- 添加加盟商尾部 -->
      <div>
        <h1 class="form_table_h3">结算账号</h1>
        ...
        <el-form-item label="姓名"  v-if="radio=='1'" prop="username">
          <el-input v-model="ruleForm.username" placeholder='请输入姓名'></el-input>
        </el-form-item>
        <el-form-item label="姓名"  v-if="radio=='2'" prop="companyName">
...
export default {
  data() {
    ...
    return {
      ...
      rules1:{
        cityId:[{ required: true, message: "请选择加盟地区", trigger: "change" }],
        ...
        dayList:[{type:"array",required: true, message: "请选择结算日", trigger: "change" }],
      },
      pickerOptions0: {
        disabledDate(time) {
  ...
  mounted: function() {
    $(".el-form.demo-ruleForm1").css("float","none")
  ...
  methods:{
    ...
    clickChange(){
      ...
      var that = this
        ...
        setTimeout(function(){
          if($("#div .is-required.is-error")){
                $("#div .is-required.is-error").removeClass("is-error")
              }
          if($("div.error-list")){
            $("div.error-list").removeClass(".error-list")
          }
          if($("div.el-form-item__error")){
            console.log(3333333)
            $("div.el-form-item__error").remove()
          }
          if($("div.el-form-item.is-error")){
            $("div.el-form-item.is-error").removeClass("is-error")
          }
        },200)
    ...
    checkJoinMode(joinMode){
     // 清空中间重复部分数据
     this.$refs[newFormObject].resetFields()
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$refs[newFormObject].validate(valid => {
            if (valid) {
