             ...
             <div id="editpartner_form">
              <el-form :model="editAccount" :rules="editRules"  ref="editAccount" label-width="110px" class="demo-ruleForm">
                ...
                <el-form-item label="授权费率" prop="licenseFeeRate">
                  <el-input v-model="editAccount.licenseFeeRate" placeholder='请输入授权费率'></el-input><span style="margin-left:5px;">%</span>
                </el-form-item>
                <el-form-item label="结算周期" prop='wType'>
                  <el-radio-group v-model="editAccount.wType">
...
export default {
  ...
  methods: {
    ...
    openEdit(row,index) {
      ...
      console.log(row)
      this.editAccount = Object.assign({},row,{editIndex:index},{percent: (row.percent*100).toFixed(2)},{licenseFeeRate: (row.licenseFeeRate*100).toFixed(2)},{cardType:row.cardType===0?'居民身份证':'护照'},{provinceName: row.provinceName},{cityName: row.cityName},{areaName: row.areaName},{wType: row.wType === 0?'自然月':'自然周(周一到周日)'})
        ...
        .then(() => {
          ...
          .end((error,res)=>{
            if(error){
              ...
              var code = JSON.parse(res.text).resultCode
              if(code===0){
                   that.tableData.splice(this.editAccount.editIndex,1,Object.assign({},this.editAccount,{joinTime:moment(this.editAccount.joinTime).format('YYYY-MM-DD')}))
                      that.dialogVisible = false
                       that.loadData()
                       that.currentPage3 = 1;
                    this.checkLogin(res)
